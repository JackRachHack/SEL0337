...
int i = 0;
int control = 0;                         // Variável para controlar o acesso ao loop de recepção e o if de envio
...
void loop() 
{
    ...
    while ((Esp8266.available() > 0) && control == 0) {  // Verificando se a porta está sendo usada e, se sim, coleta os dados
        float val = Esp8266.parseFloat();
    ...
    if (control == 1) { // Verifica, através do dado identificador, se é realmente um dado de um dos módulos                                        
        ...
        if (!falha){ // Se não falhou mantém Led pagado
          ...
          delay(10000); // Espera 30 segundos para enviar o próximo dado
